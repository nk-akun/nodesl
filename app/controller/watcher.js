...
class WatcherController extends Controller {
  ...
  async update(){
    ...
    client.connect()
    let organizationid = this.ctx.params.organizationid
    let  data = await client.query("update ti_organization set watcherid = " + "'" + this.ctx.request.body.watcher +"'"+" where organizationid = " + "'" + organizationid + "'")
    this.ctx.body = {msg:"更新watcher成功",data:this.ctx.request.body.watcher}
    this.ctx.status = 200;
  }
}
